package com.example.focourseAS.Security.Filter;
        ...
        if(request.getServletPath().equals("/focourse/login")){
            ...
            if(authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
              try {
                  ...
                  String[] roles = decodedJWT.getClaim("apiRoles").asArray(String.class);
                  Collection<SimpleGrantedAuthority> authorities = new ArrayList<>();
                  stream(roles).forEach(role-> {
                      authorities.add(new SimpleGrantedAuthority(role));
                  });
                  System.out.println("The Security passed this stage");
